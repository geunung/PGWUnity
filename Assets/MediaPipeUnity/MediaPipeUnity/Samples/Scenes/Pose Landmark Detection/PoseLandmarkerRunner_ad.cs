...
namespace Mediapipe.Unity.Sample.PoseLandmarkDetection
{
    public class PoseLandmarkerRunner_ad : VisionTaskApiRunner<PoseLandmarker>
    {
        ...
        // ✅ 추가: 팔 3점 Z 포함 getter
        public bool TryGetLeftArm3Points01Z(out Vector3 shoulder, out Vector3 elbow, out Vector3 wrist)
        {
            shoulder = elbow = wrist = default;
            lock (_poseLock)
            {
                if (!_hasPose || _lm0 == null || _lm0.Length <= L_WRIST) return false;
                shoulder = _lm0[L_SHOULDER];
                elbow = _lm0[L_ELBOW];
                wrist = _lm0[L_WRIST];
                return true;
            }
        ...
        public bool TryGetRightArm3Points01Z(out Vector3 shoulder, out Vector3 elbow, out Vector3 wrist)
        {
            shoulder = elbow = wrist = default;
            lock (_poseLock)
            {
                if (!_hasPose || _lm0 == null || _lm0.Length <= R_WRIST) return false;
                shoulder = _lm0[R_SHOULDER];
                elbow = _lm0[R_ELBOW];
                wrist = _lm0[R_WRIST];
                return true;
            }
