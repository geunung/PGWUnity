...
{
    public class MP_ClothRigDriver_ad : MonoBehaviour
    {
        [Header("Required")]
        ...
        void LateUpdate()
        {
            ...
            if (autoDepth)
            {
                float raw = baseDepthMeters * (refShoulderWidth01 / shoulderWidth01);
                ...
                if (stabilizeDepth)
                {
                    ...
                    // Optional: limit popping speed (meters/sec)
                    if (limitDepthStep)
                    {
            ...
            {
                // Keep smoothed depth aligned when switching back/forth
